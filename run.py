...
def introduction():
    ...
    print(
        "Depending on which location you'll pick you'll get a different story"
        )
...
def display_goodevil():
    """
    Display 2 options, the previous user inputs(choice)
    fills the words this time
    """
...
word_list = {
    'person': f"Enter a {colorize("persons name", 'red')}:",
...
def story_forest():
    ...
    print(
        ...
        f"It granted them incredible"
        f"{colorize(inputs['adjective'], 'magenta')}\n"
...
def story_good(inputs):
    print(
        f"==================\n"
        f"a {colorize(inputs['title'], 'blue')} named "
        f"{colorize(inputs['person'], 'red')}\n"
        f"stumbled upon a wondrous {colorize(inputs['object'], 'yellow')}.\n"
        f"It bestowed upon them noble and "
        f"{colorize(inputs['adjective'], 'magenta')}\n"
        f"powers and transported them to an "
        f"enchanting underwater utopia.\n"
        f"There, they encountered a {colorize(inputs['creature'], 'cyan')}"
        f" in distress.\n"
...
def story_evil(inputs):
    print(
        f"==================\n"
        f"{colorize(inputs['title'], 'blue')} known as "
        f"{colorize(inputs['person'], 'red')}\n"
        f"stumbled upon a malevolent {colorize(inputs['object'], 'yellow')}.\n"
        f"It bestowed upon them dark and "
        f"{colorize(inputs['adjective'], 'magenta')}\n"
        f"powers and whisked them away to a cursed underwater fortress.\n"
        f"There, they encountered a {colorize(inputs['creature'], 'cyan')}"
        f" in distress.\n"
        f"{colorize(inputs['person'], 'red')}\n"
        f"exploited the situation for their own gain, using the"
        f" {colorize(inputs['creature'], 'cyan')}\n"
